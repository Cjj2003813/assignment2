...
/*        String finaltitle = "";
        ...
        if(rows==1){
            ...
            map.put(ConstVar._KEY_MESSAGE_, "项目提交成功");
            return new ModelAndView(new MappingJackson2JsonView(),map);
        }else{
            Map<String,Object> map = new HashMap<String,Object>();
            map.put(ConstVar._KEY_CODE_, ConstVar._ERROR_COMMON_);
            map.put(ConstVar._KEY_MESSAGE_, "未知错误，项目提交失败");
            return new ModelAndView(new MappingJackson2JsonView(),map);
        }
    }
    ...
    public ModelAndView viewReport(@RequestBody int id, HttpSession session){
        ...
        map.put(ConstVar._KEY_MESSAGE_, "只能查看自己的项目，请在对应菜单中查看老师分享的项目");
        return new ModelAndView(new MappingJackson2JsonView(),map);
    }
    ...
    public ModelAndView editReport(@RequestBody UpdateReportParam updateReportParam, HttpSession session){
        ...
        if(old!=null) {
            ...
            if(oldstudent!=currentstudent) {
                Map<String,Object> map = new HashMap<String,Object>();
                map.put(ConstVar._KEY_CODE_, ConstVar._ERROR_COMMON_);
                map.put(ConstVar._KEY_MESSAGE_, "请不要篡改他人的项目，你的行为已经被系统记录");
                return new ModelAndView(new MappingJackson2JsonView(),map);
            }
        }
        //开始更新
        ...
        if(rows==1){
            Map<String,Object> map = new HashMap<String,Object>();
            map.put(ConstVar._KEY_MESSAGE_, "项目更新成功");
            return new ModelAndView(new MappingJackson2JsonView(),map);
        }else{
            Map<String,Object> map = new HashMap<String,Object>();
            map.put(ConstVar._KEY_CODE_, ConstVar._ERROR_COMMON_);
    ...
    public ModelAndView shareToPerson(@RequestBody ShareToPersonParam shareToPersonParam, HttpSession session){
        ...
        if(usertype==ConstVar._TEACHER_){
            ...
            if(rows==1){
                ...
                motification.setTitleMo("您收到一篇新的项目推荐");
                motification.setContentMo(teacherMapper.selectByPrimaryKey(currentuser).getName()+"给您分享了一篇项目，请在分享项目中查阅");
                motification.setReadMo(-1);
...
/*            Map<String,Object> map = new HashMap<String,Object>();
            map.put(ConstVar._KEY_CODE_, ConstVar._ERROR_COMMON_);
            map.put(ConstVar._KEY_MESSAGE_, "没有分享给你的项目");
            return new ModelAndView(new MappingJackson2JsonView(),map);*/
            ModelAndView mv = new ModelAndView();
